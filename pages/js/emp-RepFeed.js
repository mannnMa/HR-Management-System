...
class HRDataManager {
    ...
    // ... (keep all other HRDataManager methods unchanged) ...
}
...
// ===== FORM UTILITIES =====
function hideAllForms() {
    [trainingContainer, leaveContainer, feedbackContainer, reportContainer].forEach(container => {
        if (container) container.classList.add('form-hidden');
    });
}

function resetAllForms() {
    [trainingForm, leaveForm, feedbackForm, reportForm].forEach(form => {
        if (form) form.reset();
    });
}
...
if (trainingForm) {
    trainingForm.addEventListener('submit', (e) => {
        ...
        // Save data
        hrDataManager.saveTrainingRequest(data);
...
if (leaveForm) {
    leaveForm.addEventListener('submit', (e) => {
        e.preventDefault();
        // ... (similar structure to training form)
    });
}

// ... (similar handlers for feedback and report forms)
    ...
    if (viewButtons.length > 0) {
        viewButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                switch(formType) {
                    case 'training':
                        if (trainingContainer) trainingContainer.classList.remove('form-hidden');
                        break;
                    // ... (other cases)
                }
            });
